...
void Player::Update() {
	shoot_timer -= Time::Instance().DeltaTime() * fire_rate_multiplier;
	...
	{
		Entity* other_entity = other->GetOwner();
		if (other_entity->GetName() != "Enemy")
		{
			continue;
		}
		if (Item* item; (item = (Item*)(other_entity->GetComponent("SpeedBoostItem"))) ||
			(item = (Item*)(other_entity->GetComponent("FireRateItem"))) ||
			(item = (Item*)(other_entity->GetComponent("ExtraLifeItem")))
			) {
			item->SetPlayerAttrib(this);
		}
